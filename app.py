...
def dashboard():
    ...
    <head>
        ...
        <style>
            ...
            .last-update { color: #4ecdc4; font-size: 0.85rem; margin-top: 0.5rem; }
    ...
    <body>
        ...
        <div class="last-update">Last arb: <span id="last-arb">Never</span></div>

        <script>
            // Track last arb in memory (resets on page reload)
            let lastArb = 'Never';
            document.getElementById('last-arb').textContent = lastArb;
            ...
            async function update() {
                try {
                    ...
                    if (data.length === 0) {
                        ...
                        // Update last arb time (in memory only)
                        lastArb = new Date().toLocaleString();
                        document.getElementById('last-arb').textContent = lastArb;
